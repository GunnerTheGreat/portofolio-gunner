'use client';

import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { FaJava } from 'react-icons/fa';
import {
  SiAdobephotoshop, SiCoreldraw, SiAdobeillustrator, SiAdobepremierepro,
  SiAdobedreamweaver, SiAdobexd, SiFigma, SiAffinitydesigner, SiCanva,
  SiNextdotjs, SiReact, SiVuedotjs, SiTailwindcss, SiJavascript,
  SiTypescript, SiPhp, SiMysql, SiMongodb
} from 'react-icons/si';


const AbletonIcon = (props) => (
  <svg role="img" viewBox="0 0 24 24" fill="currentColor" {...props}>
    <title>Ableton Live</title>
    <path d="M1.6 6.4h3.2v11.2H1.6V6.4Zm6.4 0H11.2v11.2H8V6.4Zm6.4 0H17.6v11.2H14.4V6.4Zm6.4 0H24v11.2H20.8V6.4Z" />
  </svg>
);

const FLStudioIcon = (props) => (
  <svg
    role="img"
    viewBox="0 0 50.8 50.8"
    fill="none"
    stroke="currentColor"
    strokeWidth="3.175"
    strokeLinecap="round"
    strokeLinejoin="round"
    {...props}
  >
    <title>FL Studio</title>
    <path d="M17.595 16.91c-1.067 3.253-2.134 6.506-2.552 10.264-.418 3.758-.188 8.02.234 10.853.421 2.832 1.035 4.235 1.933 5.371.898 1.137 2.08 2.007 4.053 1.36 1.973-.647 4.735-2.812 7.083-5.566 2.347-2.754 4.28-6.097 5.43-8.772 1.152-2.675 1.522-4.681 1.892-6.688M17.595 16.91c-1.14.188-2.281.376-2.556-.251-.275-.627.316-2.07 1.277-3.326.96-1.257 2.291-2.33 3.725-2.843 1.435-.513 2.972-.467 4.339-.084 1.367.383 2.564 1.103 3.485 1.84.921.738 1.565 1.493 2.21 2.247" />
    <path d="M30.074 14.493c1.303-.119 2.606-.238 3.868-.022 1.262.217 2.483.77 3.444 1.65.961.88 1.662 2.086 2.043 3.385.381 1.3.443 2.693.204 3.944-.24 1.252-.78 2.363-1.491 2.386-.711.024-1.592-1.04-2.474-2.104m-5.11-11.175c.207-.8.415-1.6.735-2.339a8.662 8.662 0 0 1 1.241-2.009 8.806 8.806 0 0 1 1.548-1.499c.522-.385 1.025-.637 1.529-.89M17.595 16.91c1.864-.414 3.727-.828 4.595-.83.867 0 .738.413.773 1.421.034 1.008.232 2.61.64 3.819.407 1.209 1.024 2.023 1.658 2.539.633.515 1.284.732 2.052.67.767-.06 1.651-.398 2.608-1.226.957-.83 1.987-2.149 2.638-2.573.65-.424.922.046 1.377.696.455.65 1.094 1.478 1.732 2.306" />
  </svg>
);

const chunkArray = (arr, size) => {
  const results = [];
  for (let i = 0; i < arr.length; i += size) {
    results.push(arr.slice(i, i + size));
  }
  return results;
};

const DESIGN_STACK = [
  { name: 'Photoshop', icon: SiAdobephotoshop },
  { name: 'CorelDRAW', icon: SiCoreldraw },
  { name: 'Illustrator', icon: SiAdobeillustrator },
  { name: 'Premiere Pro', icon: SiAdobepremierepro },
  { name: 'DreamWeaver', icon: SiAdobedreamweaver },
  { name: 'Adobe XD', icon: SiAdobexd },
  { name: 'Figma', icon: SiFigma },
  { name: 'Affinity', icon: SiAffinitydesigner },
  { name: 'Canva', icon: SiCanva },
  { name: 'Ableton Live', icon: AbletonIcon },
  { name: 'FL Studio', icon: FLStudioIcon },
];

const DEV_STACK = [
  { name: 'Next.js', icon: SiNextdotjs },
  { name: 'React', icon: SiReact },
  { name: 'Vue.js', icon: SiVuedotjs },
  { name: 'Tailwind', icon: SiTailwindcss },
  { name: 'JavaScript', icon: SiJavascript },
  { name: 'TypeScript', icon: SiTypescript },
  { name: 'PHP', icon: SiPhp },
  { name: 'MySQL', icon: SiMysql },
  { name: 'MongoDB', icon: SiMongodb },
  { name: 'Java', icon: FaJava },
];

export default function HeroIcons() {
  const [currentIndex, setCurrentIndex] = useState(0);

  const designChunks = chunkArray(DESIGN_STACK, 5);
  const devChunks = chunkArray(DEV_STACK, 5);

  const slides = [
    ...designChunks.map(items => ({ title: "[ CREATIVE_TOOLS ]", items, color: "text-[#888]" })),
    ...devChunks.map(items => ({ title: "[ TECH_STACK ]", items, color: "text-[#555]" }))
  ];

  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % slides.length);
    }, 4000);

    return () => clearInterval(interval);
  }, [slides.length]);

  const currentSlide = slides[currentIndex];

  return (
    <div className="mt-16 flex flex-col items-center min-h-[160px] w-full">
      <AnimatePresence mode="wait">
        <motion.p
          key={currentSlide.title}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className={`text-xs md:text-sm uppercase tracking-widest font-mono font-bold mb-8 ${currentSlide.color}`}
        >
          {currentSlide.title}
        </motion.p>
      </AnimatePresence>

      <div className="relative w-full max-w-4xl px-4">
        <AnimatePresence mode="wait">
          <motion.div
            key={currentIndex}
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -20 }}
            transition={{ duration: 0.5 }}
            className="flex flex-wrap justify-center gap-8 md:gap-12"
          >
            {currentSlide.items.map((item) => (
              <div key={item.name} className="group relative flex flex-col items-center">
                <item.icon
                  className="w-8 h-8 md:w-10 md:h-10 transition-all duration-300 group-hover:scale-125 text-[#444] group-hover:text-[#ff1a1a] drop-shadow-[0_0_5px_rgba(255,26,26,0)] group-hover:drop-shadow-[0_0_15px_rgba(255,26,26,0.5)]"
                />

                <span className="absolute -bottom-10 text-[10px] uppercase font-mono tracking-widest opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap px-3 py-1.5 rounded-none border border-[#ff1a1a] bg-[#050505] text-[#ff1a1a] shadow-sm font-semibold pointer-events-none">
                  {item.name}
                </span>
              </div>
            ))}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
  );
}
